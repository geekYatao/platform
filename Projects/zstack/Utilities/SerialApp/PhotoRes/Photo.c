...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, PHOTO_READ_EVT); //启动光照检测
          }
        break;
      default:
        break;
  ...
  if ( events & PHOTO_READ_EVT )  //查看光敏电阻状态
  {
    PhotoResult = HalAdcRead (HAL_ADC_CHANNEL_0 ,HAL_ADC_RESOLUTION_12);//0-512
    UART0_Format.Command = 0x01;
    UART0_Format.Data[0] = PhotoResult>>8;
    UART0_Format.Data[1] = PhotoResult; 
    UART0_Format.Data[2] = 0;
    UART0_Format.Data[3] = 0;
    UART0_Format.Data[4] = 0;
    UART0_Format.Data[5] = 0;
    osal_set_event(SerialApp_TaskID, SERIALAPP_SEND_EVT);
    osal_start_timerEx(SerialApp_TaskID, PHOTO_READ_EVT, 3000);
    
    return ( events ^ PHOTO_READ_EVT );
  }
