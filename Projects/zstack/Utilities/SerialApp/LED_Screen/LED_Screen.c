  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          {
            Device_type = DeviceType_router;
            HalLedBlink(HAL_LED_2,5,50,200);
            osal_set_event(SerialApp_TaskID, PERIOD_EVT); //启动周期消息
            LedView("中软国际移动互联网综合实训平台 "); 
          }
        break;
...
{ 
  ...
  case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据
    if((pkt->cmd.Data[0]==0xcc)&&(pkt->cmd.Data[1]==0xee)) //校验包头包尾
    {
      if(pkt->cmd.Data[3] == LED_Screen)
      {
        if(pkt->cmd.Data[4] == 0x01)    
        {
          LedView(&pkt->cmd.Data[5]);
        }       
      }
    }
    break;
...
void LedView(uint8 *view){
    uint16 length;
    uint8 Led[50] = {0xaa, 0x55, 0x01, 0x01, 0x04, 0x00, 
                     0x02, 0x01, 0x12, 0x07, 0x01, 0x00, 
                     0x01, 0x00, 
                     0x06, 0x08, 0x03, 0x00, 0x33, 0xca, 0xab,0xfa};//CK16C控制卡需要将第11位改为11
    uint8 *pLed;
    uint8 i = 0;
    pLed = &Led[14];
    pLed += 5;
    osal_memcpy( pLed, view, strlen((const char *)view));
    length = strlen((char const  *)view);
    length --;  //CK16C控制卡需要将此句注释
    Led[9] = length -1;  //CK16C控制卡需要将此句注释
    Led[14] = length >> 8;
    Led[14] = length + 4;
    pLed = &Led[11];
    Led[15 + length + 4] = 0;
    for (i = 0; i < length + 8; i ++){
      Led[15 + length + 4] ^= pLed[i];
    }
    HalLedBlink(HAL_LED_2,3,50,200);
    HalUARTWrite(SERIAL_APP_PORT, Led, 16 + length + 4);
}
