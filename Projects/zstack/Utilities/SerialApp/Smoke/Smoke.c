...
static uint8 SmokeStatusOld;
static uint8 SmokeStatusNow;
...
static void PhotoInit(void) //P00  AD   P01   开关
{
  ...
  if(SMOKE_GPIO == HIGH)
    SmokeStatusNow = SMOKE_YES;
  else
    SmokeStatusNow = SMOKE_NO;
}
/*********************************************************************
 * @fn      SerialApp_Init
 *
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, SMOKE_READ_EVT); //启动光照检测
          }
        break;
      default:
        break;
  ...
  if ( events & SMOKE_READ_EVT )  //查看光敏电阻状态
  {
    SmokeStatusOld = SmokeStatusNow;  //保存当前状态
    if(SMOKE_GPIO == HIGH)
    {
      SmokeStatusNow = SMOKE_YES;
    }
    else if(SMOKE_GPIO == LOW)
    {
      SmokeStatusNow = SMOKE_NO;
    }
    
    if(SmokeStatusNow != SmokeStatusOld) //如果当前状态和上一个状态发生变化，发送消息给网关
    {
      UART0_Format.Command = 0x01;
      if(SmokeStatusNow == SMOKE_YES)
      {
        UART0_Format.Data[0] = 0x01;
        UART0_Format.Data[1] = 0x00;
      }
      else if(SmokeStatusNow == SMOKE_NO)
      {
        UART0_Format.Data[0] = 0x00;
        UART0_Format.Data[1] = 0x00;       
      }
      UART0_Format.Data[2] = 0; //节点自身网络地址 高字节
      UART0_Format.Data[3] = 0; //节点自身网络地址 低字节
      UART0_Format.Data[4] = 0; //父节点网络地址 高字节
      UART0_Format.Data[5] = 0; //父节点身网络地址 低字节 
      osal_set_event(SerialApp_TaskID, SERIALAPP_SEND_EVT);
    }
    
    osal_start_timerEx(SerialApp_TaskID, SMOKE_READ_EVT, 200);
    
    return ( events ^ SMOKE_READ_EVT );
  }
