...
void SerialApp_Init( uint8 task_id )
{
  ...
  UART0_Format.NodeID   = Router_LED;
...
void SerialApp_ProcessMSGCmd( afIncomingMSGPacket_t *pkt )  //处理接收到的RF消息
{
  ...
  case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据
    ...
    {
      if(receiveData->NodeID == Router_LED)
      {
        if(receiveData->Command == 0x01)    //
        {
          if(receiveData->Data[0] == PhotoRes)  //光敏传感器节点
          {
            if(receiveData->Data[1] == 0x01)
            {
              P0_0 = 1;
            }
            else if(receiveData->Data[1] == 0x00)
            {
              P0_0 = 0;
            }            
          }
          else if(receiveData->Data[0] == Ir_sw)  //红外光电传感器节点
          {
            if(receiveData->Data[1] == 0x01)
            {
              P0_1 = 1;
          ...
          else if(receiveData->Data[0] == Sound)  //声音
          {
            if(receiveData->Data[1] == 0x01)
            {
              P0_2 = 1;
          ...
          else if(receiveData->Data[0] == Flame)  //火焰
          {
            if(receiveData->Data[1] == 0x01)
            {
              P0_3 = 1;
          ...
          else if(receiveData->Data[0] == Ir_safe) //红外对射
          {
            if(receiveData->Data[1] == 0x01)
            {
              P0_4 = 1;
          ...
          else if(receiveData->Data[0] == Shake)  //震动
          {
            if(receiveData->Data[1] == 0x01)
            {
              P0_5 = 1;
