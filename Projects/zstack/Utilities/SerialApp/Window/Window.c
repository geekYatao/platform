...
void SerialApp_ProcessMSGCmd( afIncomingMSGPacket_t *pkt )  //处理接收到的RF消息
{
  ...
  switch ( pkt->clusterId )
  {
   case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据    
     ...
     {
       if(receiveData->NodeID == Window) //地址
       {
         if(receiveData->Command == OPEN_WINDOW)  //开窗帘1
         {
             Relay_1 = ON;
             MicroWait(100000);
          Relay_1 = OFF;
         ...
         else if(receiveData->Command == CLOSE_WINDOW)//开窗帘2
         {
             Relay_2 = ON;
                          MicroWait(100000);
          Relay_2 = OFF;
