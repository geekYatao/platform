...
static uint8 GasStatusOld;
static uint8 GasStatusNow;
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, GAS_READ_EVT); //启动光照检测
          }
        break;
      default:
        break;
  ...
  if ( events & GAS_READ_EVT )  //查看光敏电阻状态
  {
    GasStatusOld = GasStatusNow;  //保存当前状态
    if(GAS_GPIO == HIGH)
    {
      GasStatusNow = GAS_YES;
    }
    else if(GAS_GPIO == LOW)
    {
      GasStatusNow = GAS_NO;
    }
    
    if(GasStatusNow != GasStatusOld) //如果当前状态和上一个状态发生变化，发送消息给网关
    {
      UART0_Format.Command = 0x01;
      if(GasStatusNow == GAS_YES)
      {
        UART0_Format.Data[0] = 0x01;
        UART0_Format.Data[1] = 0x00;
      }
      else if(GasStatusNow == GAS_NO)
      {
    ...
    osal_start_timerEx(SerialApp_TaskID, GAS_READ_EVT, 200);
    
    return ( events ^ GAS_READ_EVT );
  }
