...
void SerialApp_ProcessMSGCmd( afIncomingMSGPacket_t *pkt )  //处理接收到的RF消息
{
  ...
  switch ( pkt->clusterId )
  {
   case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据    
     ...
     {
       if(receiveData->NodeID == SmartLamp) //地址
       {
         if(receiveData->Command == ON_LAMP1)  //01
         {
           Relay_2 = ON;
         }
         else if(receiveData->Command == OFF_LAMP1) //02
         {
           Relay_2 = OFF;
         }
       }
       if(receiveData->NodeID == SmartLamp1)
       {
         if(receiveData->Command == ON_LAMP2)  //01
         {
           Relay_2 = ON;
         }
         else if(receiveData->Command == OFF_LAMP2) //02
         {
           Relay_2 = OFF;
         }
       }
