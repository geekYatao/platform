...
static uint8 RFIDStatusNew;
static uint8 RFIDStatusOld;
/*********************************************************************
 * LOCAL FUNCTIONS
 */
...
static void GPIOInit(void) //P00  AD   P01   开关
{
  P0SEL &= ~0x01;  //P00设置为通用IO
  P0DIR &= ~0x01;  //将P00设置为输入模式 
}
/*********************************************************************
 * @fn      SerialApp_Init
 *
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
  ...
  if ( events & RFID_EVT ) //发送RF消息
  {
    uint8 readCardBuf[] = {0xaa, 0xbb, 0x02, 0x20, 0x22}; // 读卡命令
    RFIDStatusOld = RFIDStatusNew;  //保存当前状态
    if(!RFID_PIN) //检测到卡片
    {
      RFIDStatusNew = READ;
    }
    ...
    {
      RFIDStatusNew = NOREAD;
    }
    if(RFIDStatusOld != RFIDStatusNew) //状态变化读卡
    {
      if(RFIDStatusNew == READ) //读卡
      {
        HalUARTWrite(SERIAL_APP_PORT, readCardBuf, sizeof(readCardBuf));
      }
    }
