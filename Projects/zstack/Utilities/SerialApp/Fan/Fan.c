...
void SerialApp_Init( uint8 task_id )
{
 ...
 // UART0_Format.NodeID   = Fan;
  UART0_Format.NodeID   = 0x20;
...
void SerialApp_ProcessMSGCmd( afIncomingMSGPacket_t *pkt )  //处理接收到的RF消息
{
  ...
  switch ( pkt->clusterId )
  {
   case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据    
     ...
     {
    //   if(receiveData->NodeID == Fan) //地址
       if(receiveData->NodeID == 0x20) //地址
       {
         
         if(receiveData->Command == ON_FAN)  //01
         {
           Relay_2 = ON;
           HalLedBlink(HAL_LED_2,10,50,200);
         }
         else if(receiveData->Command == OFF_FAN) //02
         {
           Relay_2 = OFF;
         }
       }
