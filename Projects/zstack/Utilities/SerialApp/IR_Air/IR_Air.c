  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_ROUTER) //判定当前设备类型
          {
...
void SerialApp_ProcessMSGCmd( afIncomingMSGPacket_t *pkt )  //处理接收到的RF消息
{
  ...
  switch ( pkt->clusterId )
  {
   case SERIALAPP_CLUSTERID1:  //处理各个传感器节数据    
     ...
     {
       if(receiveData->NodeID == IR_Air) //地址
       {
         HalLedBlink(HAL_LED_2,10,50,200);
         if(receiveData->Command == IR_TX)  //红外发送键值命令  发送红外命令  02
         {
           Rsp.Data[0] = IR_TX;
           Rsp.Data[1] = receiveData->Data[0];
           HalUARTWrite(SERIAL_APP_PORT, &receiveData->Data[0], 1);  //学习红外指令
         }
         else if(receiveData->Command == IR_RX) //红外接收命令  学习红外命令  01
         {
           Rsp.Data[0] = IR_RX;
           Rsp.Data[1] = receiveData->Data[0];
           Rsp.Data[2] = receiveData->Data[1];
           HalUARTWrite(SERIAL_APP_PORT, &receiveData->Data[0], 1);  //学习红外指令
           MicroWait(1000);
           HalUARTWrite(SERIAL_APP_PORT, &receiveData->Data[1], 1);  //学习红外指令
         }
