...
static afAddrType_t  PWM_AC_TxAddr;
static afAddrType_t  PWM_AC1_TxAddr;
static afAddrType_t  PWM_AC2_TxAddr;
static afAddrType_t  PWM_AC3_TxAddr;
...
void SerialApp_Init( uint8 task_id )
{
  ...
  UART0_Format.Header_1 = 0xcc;
  UART0_Format.Header_2 = 0xee;
  ...
  PWM_AC_TxAddr.addrMode =(afAddrMode_t) AddrBroadcast; //发送地址初始化
  PWM_AC_TxAddr.endPoint = SERIALAPP_ENDPOINT;
  PWM_AC_TxAddr.addr.shortAddr = 0xffff;
  
  PWM_AC1_TxAddr.addrMode =(afAddrMode_t) AddrBroadcast; //发送地址初始化
  PWM_AC1_TxAddr.endPoint = SERIALAPP_ENDPOINT;
  PWM_AC1_TxAddr.addr.shortAddr = 0xffff;
  
  PWM_AC2_TxAddr.addrMode =(afAddrMode_t) AddrBroadcast; //发送地址初始化
  PWM_AC2_TxAddr.endPoint = SERIALAPP_ENDPOINT;
  PWM_AC2_TxAddr.addr.shortAddr = 0xffff;
  
  PWM_AC3_TxAddr.addrMode =(afAddrMode_t) AddrBroadcast; //发送地址初始化
  PWM_AC3_TxAddr.endPoint = SERIALAPP_ENDPOINT;
  PWM_AC3_TxAddr.addr.shortAddr = 0xffff;
  TXPOWER = 0xf5;
}
...
static void SerialApp_CallBack(uint8 port, uint8 event)
{
...
#endif
  {
    ...
    {
      if(SerialApp_TxLen == 38)
      {
        SerialApp_OTAData(&SerialApp_TxAddr, SERIALAPP_CLUSTERID1, SerialApp_TxBuf, 38);
      }
      else
      {
        ...
        if((p->Header_1==0xcc)&&(p->Header_2==0xee)&&(p->Tailer==0xff))//包头包尾校验
        {
          if(p->NodeID == PWM_AC)
          {
            SerialApp_OTAData(&PWM_AC_TxAddr,SERIALAPP_CLUSTERID1, p, sizeof(UART_Format));
          }
          else if(p->NodeID == PWM_AC1)
          {
            SerialApp_OTAData(&PWM_AC1_TxAddr,SERIALAPP_CLUSTERID1, p, sizeof(UART_Format));
          }
          else if(p->NodeID == PWM_AC2)
          {
            SerialApp_OTAData(&PWM_AC2_TxAddr,SERIALAPP_CLUSTERID1, p, sizeof(UART_Format));
          }
          else if(p->NodeID == PWM_AC3)
          {
            SerialApp_OTAData(&PWM_AC3_TxAddr,SERIALAPP_CLUSTERID1, p, sizeof(UART_Format));
          }
          else if(p->NodeID != Coor) //确定不是发送给网关的消息
          {
