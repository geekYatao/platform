...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, FLAME_READ_EVT); //启动光照检测
          }
        break;
      default:
        break;
  ...
  if ( events & FLAME_READ_EVT )  
  {
    FlameResult[0] = HalAdcRead (HAL_ADC_CHANNEL_0 ,HAL_ADC_RESOLUTION_10);//0-512
    FlameResult[1] = HalAdcRead (HAL_ADC_CHANNEL_1 ,HAL_ADC_RESOLUTION_10);//0-512
    UART0_Format.Command = 0x01;
    UART0_Format.Data[0] = FlameResult[0] >> 8;
    UART0_Format.Data[1] = FlameResult[0]; 
    UART0_Format.Data[2] = FlameResult[1] >> 8;
    UART0_Format.Data[3] = FlameResult[1];  
    osal_set_event(SerialApp_TaskID, SERIALAPP_SEND_EVT);   
    osal_start_timerEx(SerialApp_TaskID, FLAME_READ_EVT, 1000);
    return ( events ^ FLAME_READ_EVT );
  }
