...
static uint8 DopplerStatusOld;
static uint8 DopplerStatusNow;
...
UINT16 SerialApp_ProcessEvent( uint8 task_id, UINT16 events )
{
    ...
    while ( (MSGpkt = (afIncomingMSGPacket_t *)osal_msg_receive( SerialApp_TaskID )) )
    {
      ...
      case ZDO_STATE_CHANGE:
          ...
          if(SampleApp_NwkState == DEV_END_DEVICE) //判定当前设备类型
          {
            ...
            osal_set_event(SerialApp_TaskID, DOPPLER_READ_EVT); //启动光照检测
          }
        break;
      default:
        break;
  ...
  if ( events & DOPPLER_READ_EVT )  //查看光敏电阻状态
  {
    DopplerStatusOld = DopplerStatusNow;  //保存当前状态
    if(DOPPLER_GPIO == HIGH)
    {
      DopplerStatusNow = DOPPLER_YES;
    }
    ...
    if(DopplerStatusNow != DopplerStatusOld) //如果当前状态和上一个状态发生变化，发送消息给网关
    {
